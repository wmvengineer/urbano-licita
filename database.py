...
def create_pagarme_checkout(user_data, plan_tag, plan_name, amount_float, address_data, user_phone, user_email):
    ...
    Se o link não vier na criação do pedido, gera um checkout avulso para o ID criado.
    """
    ...
    user_address = {
        ...
        "zip_code": clean_cep, 
        "line_1": line_1_fmt[:128], 
        "line_2": "Sem complemento"
    }

    # --- 2. TENTATIVA 1: CRIAR PEDIDO + CHECKOUT JUNTOS ---
    ...
    try:
        ...
        try:
            ...
            return False, f"Erro Fatal Pagar.me: {response.status_code} - {response.text}", None
        ...
        if response.status_code == 200:
            ...
            # CASO A: Link veio corretamente
            if checkouts:
                return True, checkouts[0].get('payment_url'), order_id
            
            # CASO B: Link não veio (Seu problema atual). Vamos gerar manualmente.
            else:
                print(f"--- PEDIDO {order_id} CRIADO SEM LINK. GERANDO CHECKOUT AVULSO... ---")
                
                # Endpoint específico para criar checkout em pedido existente
                url_rescue = f"https://api.pagar.me/core/v5/orders/{order_id}/checkouts"
                ...
                resp_rescue = requests.post(url_rescue, headers=get_pagarme_auth(), json=payload_rescue)
                json_rescue = resp_rescue.json()
                
                if resp_rescue.status_code == 200:
                    return True, json_rescue.get('payment_url'), order_id
                else:
                    st.error("Falha ao gerar link de pagamento (Segunda tentativa).")
                    st.json(json_rescue)
